...

{
    ...
    *  check for duplicates in row, column and the 3x3 grid
    ...
    private boolean ForwardCheck(int[][] board, int row, int column, int number) {
    ...
    {
        return !checkRow(board, row, number)&& !checkColumn(board, column, number) && !checkBox(board, row, column, number) && !ForwardCheck(board, row, column, number);
    }
